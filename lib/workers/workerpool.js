...
class WorkerPool extends EventEmitter {
  ...
  async mine(data, target, min, max) {
    ...
    return result.nonce;//result.nonce
  }
...
class Worker extends EventEmitter {
  ...
  execute(packet, timeout) {
    //console.log('asdadsdasdasdasdadsdasda');
    return new Promise((resolve, reject) => {
  ...
  _execute(packet, timeout, resolve, reject) {
    ...
    try {
      // Start the job
      const result = job.start(timeout);
      console.log(result);
      // If the job completed successfully
      if (result === undefined) {
        job.resolve(result); // Resolve the job with the result
      } else {
        // Handle cases where result might be undefined but the job didn't throw an error
        job.reject(new Error('Job did not provide a result.'));
      }
    } catch (error) {
      // If the job threw an error
      job.reject(error); // Reject the job with the error
    }
